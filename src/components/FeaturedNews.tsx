      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {newsItems.map((item) => (
            <article
              key={item.id}
              className="bg-white rounded-3xl p-6 sm:p-7 shadow-lg hover:shadow-2xl transition-all duration-300 flex flex-col justify-between border border-black/5 hover:-translate-y-1 group"
            >
              <div className="space-y-4">
                ...
                {/* News Title */}
                <h3 className="text-base sm:text-lg font-bold text-neutral-900 leading-snug group-hover:text-[#034e9f] transition-colors">
                  {item.title}
                </h3>
